...
def feature_extract(df: DataFrame, tweet: str):
    ...
    df['is_retweet'] = df[tweet].apply(lambda t: 1 if re.search(r'[Rr][Tt].@\S+', t) else 0)
...
def clean(tweet: str):
    ...
    # remove retweet tags
    no_retweets = re.sub(r"[Rr][Tt].@\S+", " ", new)
    ...
    # remove all mentions
    no_mentions = re.sub(r"@\S+", " ", negations_fix)
    # remove all special characters
    letters_only = re.sub(r"[^a-zA-Z]", " ", no_mentions)
    # tokenize
    ...
    for col, f_type in columns:
        if col in df.columns:
            if f_type == 'tweet':
                ...
                clean_columns.append((col, f_type))
